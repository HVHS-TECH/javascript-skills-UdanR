...
const planets = [
    { x: 0, y: 0, r: 80, gravity: 0.08, color: "#3da9ff" },
    { x: 600, y: -300, r: 60, gravity: 0.06, color: "#7CFF7C" },
    { x: -700, y: 400, r: 100, gravity: 0.1, color: "#ff6b6b" }
];
...
function spawnFlame(){
    particles.push({
        ...
        vx: -Math.cos(rocket.angle)*3,
        vy: -Math.sin(rocket.angle)*3,
        life: 30,
...
function update(){
    ...
    if(keys["w"] && rocket.fuel > 0 && !rocket.landed){
...
function drawMinimap(){
    ...
    ctx.fillStyle="white";
    ctx.fillRect(
        x0 + size/2 + rocket.x*0.05,
        y0 + size/2 + rocket.y*0.05,
        3,3
    );
}
